    ...
    public sealed class EdgeLookup
    {
        ...
        public List<EdgePair> FindMatchingPairs(NeighborEdge[] probeStar, NeighborEdge[] candidateStar)
        {
            ...
            for (int candidateIndex = 0; candidateIndex < candidateStar.Length; ++candidateIndex)
            {
                ...
                {
                    if (Angle.Distance(probeStar[probeIndex].Edge.ReferenceAngle, candidateEdge.Edge.ReferenceAngle) <= MaxAngleError
                        && Angle.Distance(probeStar[probeIndex].Edge.NeighborAngle, candidateEdge.Edge.NeighborAngle) <= MaxAngleError)
                    {

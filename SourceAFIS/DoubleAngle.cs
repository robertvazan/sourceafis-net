...
namespace SourceAFIS
{
    static class DoubleAngle
    {
        ...
        public static DoublePoint ToVector(double angle) { return new DoublePoint(Math.Cos(angle), Math.Sin(angle)); }
        public static double Atan(double x, double y)
        {
            double angle = Math.Atan2(y, x);
            return angle >= 0 ? angle : angle + Pi2;
        }
        public static double Atan(DoublePoint point) { return Atan(point.X, point.Y); }
        public static double Atan(IntPoint point) { return Atan(point.X, point.Y); }
        public static double Atan(IntPoint center, IntPoint point) { return Atan(point - center); }
        public static double ToOrientation(double angle) { return angle < Math.PI ? 2 * angle : 2 * (angle - Math.PI); }
        public static double FromOrientation(double angle) { return 0.5 * angle; }
        public static double Add(double start, double delta)
        {
            double angle = start + delta;
            return angle < Pi2 ? angle : angle - Pi2;
        }
        public static double BucketCenter(int bucket, int resolution) { return Pi2 * (2 * bucket + 1) / (2 * resolution); }
        ...
        public static double Opposite(double angle) { return angle < Math.PI ? angle + Math.PI : angle - Math.PI; }
        ...
        public static bool Normalized(double angle) { return angle >= 0 && angle < Pi2; }
    }
}
